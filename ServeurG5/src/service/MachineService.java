...
public class MachineService extends UnicastRemoteObject implements IDao<Machine> {
    ...
    public List<Machine> findAll() throws RemoteException {
        ...
        try {
            session = HibernateUtil.getSessionFactory().openSession();
            tx = session.beginTransaction();
            machines = session.getNamedQuery("findAllMachineNative").list();
            tx.commit();
        } catch (HibernateException ex) {
            if (tx != null) {
                tx.rollback();
    ...
    @Override
    public List<Machine> findMachineParSalle(Salle o) throws RemoteException {
        Session session = null;
        Transaction tx = null;
        List<Machine> machines = null;
        try {
            session = HibernateUtil.getSessionFactory().openSession();
            tx = session.beginTransaction();
            machines = session.getNamedQuery("findMachineParSalle").setParameter("salle", o).list();
            tx.commit();
        } catch (HibernateException ex) {
            if (tx != null) {
                tx.rollback();
